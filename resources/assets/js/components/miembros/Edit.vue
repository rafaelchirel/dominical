  ...
  import Const from '../Const'
    ...
    methods:{
      getMiembro: function () {
        ...
        var urlApiURL = Const.apiURL + '/miembro/' + id + '/edit';
          axios.get(urlApiURL).then(response => {
                    this.miembro = response.data;
                    this.condicion = this.miembro.condicion
          });
          this.$store.state.condicionUrl = true//Inicializando condicion
      },
      update: function () {
          var url    = Const.apiURL + '/miembro/' + this.miembro.id;
          axios.put(url, this.miembro).then(response => {
            this.$router.push('/miembros')
            toastr.options.positionClass = 'toast-bottom-right';
            toastr.success('Participante Actualizado Exitosamente')
          }).catch(error   => {
      ...
      observacion: function (event){
        if (this.condicion == 'inactivo') return false
